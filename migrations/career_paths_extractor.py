...
CHARACTERISTIC_LABELS = ['CC', 'CT', 'F', 'E', 'Ag', 'Int', 'FM', 'Soc', 'A', 'B', 'M', 'Mag']
...
def extract_career_data(pdf_file):
    ...
    with pdfplumber.open(pdf_file) as pdf:
        ...
        for page_index, page in enumerate(pages):
            ...
            if page_index == 56:
                LOGGER.info("Page 57: Ignorée la carrière Cenobite est sur deux pages, elle est traitée à la main.")
                continue
            table = tables[0]
            if len(table[0]) == 12:
                ...
                for i in range(4, len(table)):
                    if "Accès" in (table[i][0] or ""):
                        ...
                        break
                if i_paths is None:
                    LOGGER.warning("Aucun chemin de carrière trouvé dans la table pour la carrière '%s'.", career_name)
                    continue
                ...
                if career_response.data:
                    ...
                    for opening in openings:
                        opening_response = supabase.rpc("search_careers", {"search_term": opening}).execute()
                        if opening_response.data:
                            path = [career_response.data[0]["id"], opening_response.data[0]["id"]]
                        else:
                            LOGGER.warning(f"Opening '{opening}' not found for career '{career_name}', check typo")
                            failed_careers.append(opening)
                            continue
                        LOGGER.debug(f"Created path: {path[0]} -> {path[1]} for career '{career_name}' and opening '{opening}'")
                        paths.append(path)
                else:
                    LOGGER.warning(f"Career '{career_name}' not found in the database, check typo")
                    failed_careers.append(career_name)
                    continue
                
                careers_data[career_response.data[0]["name"] if career_response.data else career_name] = {
...
def insert_career_data_to_db(client: Client, careers: list[JSON], json_data: dict):
    ...
    for career_name, data in json_data.items():
        ...
        if not career_id:
            LOGGER.warning(f"Attention : Carrière '{career_name}' introuvable en base de données pour l'insertion.")
            continue
    ...
    if characteristics_to_insert:
        try:
            client.table("career_characteristics").upsert(characteristics_to_insert, on_conflict="career_id,stat_code").execute()
            LOGGER.info(f"Inséré/Mis à jour {len(characteristics_to_insert)} caractéristiques dans career_characteristics.")
        except APIError as e:
            LOGGER.error(f"APIError sur career_characteristics [{e.code}]: {e.message} - {e.hint}")
        ...
        if paths_to_insert:
            try:
                ...
                client.table("career_paths").upsert(unique_paths, on_conflict="from_career_id,to_career_id").execute()
                LOGGER.info(f"Inséré/Mis à jour {len(unique_paths)} chemins dans career_paths (après déduplication).")
            except APIError as e:
                LOGGER.error(f"APIError sur career_paths [{e.code}]: {e.message} - {e.hint}")
...
if __name__ == "__main__":
    ...
    cenobite_paths = [
        ...
        [get_id_from_name(careers, cenobite_career_name), get_id_from_name(careers, "Serviteur")],
        [get_id_from_name(careers, cenobite_career_name), get_id_from_name(careers, "Vagabond")],
    ]
    ...
    LOGGER.warning(f"Nombre de carrières extraites du PDF / dans la base de données: {data_count} / {careers_count} = {data_count / careers_count:.2%}")
    if data_count != careers_count:
        LOGGER.warning("Le nombre de carrières extraites du PDF ne correspond pas au nombre de carrières dans la base de données. Voici les carrières manquantes ou en trop:")
        ...
        if missing_in_pdf:
            LOGGER.warning("Carrières manquantes dans le PDF: %s", ", ".join(missing_in_pdf))
        if missing_in_db:
            LOGGER.warning("Carrières manquantes dans la base de données: %s", ", ".join(missing_in_db))

...
import type { CharacterArmor, CharacterSkill, CharacterTalent, CharacterWeapon } from '../types/domain'
...
export async function listCharacterSkills(characterId: string): Promise<CharacterSkill[]> {
  const { data, error } = await supabase
    ...
    .select('skill_id, mastery_level, skills!inner(id, name, specialization, description)')
...
export async function listCharacterWeapons(characterId: string): Promise<CharacterWeapon[]> {
  const { data, error } = await supabase
    ...
    .select('id, weapon_id, equiped, weapons!inner(id, name, description)')
    .eq('character_id', characterId)
...
export async function listCharacterArmors(characterId: string): Promise<CharacterArmor[]> {
  const { data, error } = await supabase
    ...
    .select('id, armor_id, is_equipped, armors!inner(id, name, description, covered_locations)')
    .eq('character_id', characterId)
